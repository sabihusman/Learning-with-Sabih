...
Systems and Networking|48|Load Balancing|Send one fixed request stream through a balancer to three servers and watch round robin pile long requests onto one server while least connections keeps them level, then kill a server mid-run and see which policy copes|load-balancing
Systems and Networking|49|CAP Theorem|Replicate one key across two nodes, cut the network between them, and choose: refuse writes to stay consistent or serve both sides and let the copies diverge, the tradeoff a partition forces|cap-theorem
Systems and Networking|50|Sharding|Pick a partition key and watch rows hash into shards, then see a low-cardinality key pile most rows onto one hot shard|sharding
Systems and Networking|51|Consistent Hashing|Place keys on a hash ring, then add a node and watch only a small slice of keys move, while plain hash-mod-N would reshuffle almost everything|consistent-hashing
Systems and Networking|52|Race Conditions and Locks|Step two threads through a shared balance by hand or on auto-play, watch a bad interleaving lose an update, then turn on a lock and try to break it again|race-conditions
Systems and Networking|53|Deadlock|Two threads, two locks, grabbed in opposite orders, and watch the wait-for graph close into a circular deadlock, then switch to one lock order and see it become impossible|deadlock
Systems and Networking|54|TCP and UDP|Send the same 8 packets across an identical lossy channel and watch UDP finish fast with permanent gaps while TCP detects each drop, resends it, and holds arrivals out of order until the gap behind them fills in|tcp-and-udp
Systems and Networking|55|DNS|Resolve a name through root, TLD, and authoritative servers, watching each one hand back a referral instead of an answer, then look it up again and watch the cached answer skip the whole chain|dns
