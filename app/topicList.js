...
Databases and SQL|18|Tables and the Relational Model|Data split across tables and linked by keys: hover a row to see the primary-to-foreign-key link|relational-model
Databases and SQL|19|SELECT, WHERE and CASE|Choose columns, filter rows live, and bucket them with a CASE expression|select-where-case
Databases and SQL|20|Joins|Match rows across tables, and watch INNER, LEFT, RIGHT, and FULL change the result|joins
Databases and SQL|21|GROUP BY and Aggregation|Collapse rows into one summary per group, and see how COUNT, DISTINCT, and HAVING behave|group-by
Databases and SQL|22|Window Functions|Rank, number, and total across rows without collapsing them, and see how ties split the ranks|window-functions
Databases and SQL|23|Funnel Analysis|Count distinct sessions through each step and watch where users drop off|funnel-analysis
Object-Oriented Programming|24|Classes and Objects|Stamp objects off a class blueprint, give each its own state, then lock a field down with encapsulation|classes-and-objects
Object-Oriented Programming|25|Inheritance|Build a robot family tree and watch method lookup climb the chain until an override wins|inheritance
Object-Oriented Programming|26|Polymorphism|Send one activate() call to robots all typed as Robot and watch each run its own behavior|polymorphism
Object-Oriented Programming|27|Composition vs Inheritance|Grow an inheritance tree until it tangles, then build the same robot by snapping modules in|composition-vs-inheritance
`
