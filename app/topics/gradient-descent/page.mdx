...
The loss curve is a hand-picked toy function, one dimensional so the walk
stays visible. Everything else is computed from it for real: the slope, every
step of the update rule, the settling, and the divergence at high learning
rates all come from evaluating f and its derivative live, nothing is scripted.
