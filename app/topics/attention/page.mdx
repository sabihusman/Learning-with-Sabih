...
kind of relationship. In the grid above, each cell's shade is the relative
strength of one word attending to another, brightest for the strongest link; the
numbers are hand-set to make the lesson clear and do not add up to 1 the way a
real model's would.
