...
Run the transfer in the figure and watch both accounts change. For a moment between the debit and the credit the total dips out of balance, and that in-between state is exactly what atomicity will not let you commit. Then make it fail partway, after the first step. The transaction rolls back, the first step is undone, and the total is whole again. Atomicity guarantees the total at commit, not at every instant during the transfer.
