...
Run the transfer in the figure and watch both accounts change. The total dips out of balance for a moment mid-transfer, between the debit and the credit, which is exactly the in-flight state atomicity does not let you commit. Then make it fail partway, after the first step. The transaction rolls back, the first step is undone, and the total is back to correct.
