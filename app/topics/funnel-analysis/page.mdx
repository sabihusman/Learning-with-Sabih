...
In the figure below, you see the three funnel steps sized by their distinct session counts, the conversion percentage between each, and the SQL that produces them. Use the cohort buttons to slice the same funnel by plan or signup month; the counts and the SQL both recompute, and the n= label shows how many users the slice contains.

## Watching sessions drop off
