...
## Linking rows with keys

<RelationalModelViz />

The tables are a small hand-picked slice of the site's shared seed dataset,
four users and five sessions chosen so the key relationship is easy to see.
The row linking is real: highlights come from filtering sessions by the
clicked user's key, not from an authored mapping.
