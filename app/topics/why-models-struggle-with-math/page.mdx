...
In the figure, the multiplication model side generates its token probabilities from a formula shaped to blur more on later digits and bigger answers, the letter-counting side uses hand-authored probabilities, and the calculation side is computed for real in both.
