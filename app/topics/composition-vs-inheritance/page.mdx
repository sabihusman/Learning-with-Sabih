...
In the figure, the class counts and the duplicate tags come from the real capability lists as you add and combine modules. The broken outcome when an inheritance combination ripples is scripted to show the fragile base problem, not computed from how the classes actually behave. The figure shows that combining subclasses turns fragile; it does not model the specific reason any one combination breaks.

## Trees vs snapped-in modules
