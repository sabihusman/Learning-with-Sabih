...
The objects are real state: each card's fields live in its own object,
updates change only that object, and the private-mode refusal is an actual
guard in the code. The class blueprint panel is a description of the class,
not source code being compiled or run.
