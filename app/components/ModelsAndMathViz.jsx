...
export default function ModelsAndMathViz() {
  ...
  return (
    <Figure
      ...
      status={status}
      readouts={readouts}
      tryThis="Type your own two numbers into the multiplication problem, or keep the defaults, then press Step. The left side runs the real algorithm in plain JavaScript and is always right, for any operands you enter. The right side imitates a language model: at each step it predicts the next answer token from a probability over likely tokens and commits the most likely one, with no arithmetic underneath. Try small numbers first, where the model is usually right, then larger ones and watch the later digits get blurrier until the guess flips wrong. For multiplication those probabilities are generated by a formula tuned to blur more for later digits and bigger answers, not hand-authored and not drawn from a real model; the letter-counting problem still uses fixed, hand-authored numbers."
    >
      ...
      <p className={styles.caption}>
        ...
        has seen, which is why it can be close but wrong, and why it miscounts. For the multiplication problem,
        each digit&apos;s probabilities are generated by a formula that blurs more for later digits and bigger
        answers, deliberately shaped to demonstrate that trend, not sampled from any real model. The
        letter-counting problem still uses fixed, hand-authored numbers.
      </p>
    </Figure>
  )
